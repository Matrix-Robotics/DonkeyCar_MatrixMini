...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if cfg.DONKEY_GYM or cfg.DRIVE_TRAIN_TYPE == "MOCK":
    ...
    elif cfg.DRIVE_TRAIN_TYPE == "MatrixMini":
        from donkeycar.parts.actuator import TwoWheelSteeringThrottle, MatrixL, MatrixR #, MatrixMotor

        left_motor = MatrixL() #MatrixMotor(1)
        right_motor = MatrixR() #MatrixMotor(2)
        two_wheel_control = TwoWheelSteeringThrottle()
        ...
        # Mini1.M1.set(int(float(outputs[0]) *100))
        # Mini1.M2.set(int(float(outputs[1]) *100))
        V.add(left_motor, inputs=['left_motor_speed'])
        V.add(right_motor, inputs=['right_motor_speed'])
        # Mini1.RGB1.setRGB(0, 255, 150)
        # Mini1.RGB2.setRGB(0, 255, 150)  
